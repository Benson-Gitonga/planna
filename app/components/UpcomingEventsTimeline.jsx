...
export default function UpcomingEventsTimeline(){
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try{
                const res = await fetch('http://localhost:5000/api/admin/upcoming-events', {
                    credentials: 'include',
                });
        ...
        <div className="border-start border-2 ps-3">
          {events.map((event, i) => (
            <motion.div
              ...
              className="mb-4"
            >
              <div className="mb-1 small text-muted">{dayjs(event.event_date).format('MMM D, YYYY')}</div>
              <h6 className="mb-1">{event.event_name} {getUrgency(event.event_date)}</h6>
              <p className="mb-0 small">
                {event.start_time} - {event.end_time} @ <strong>{event.location}</strong>
              </p>
              <hr />
            </motion.div>
          ))}
